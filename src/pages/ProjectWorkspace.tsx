...
import { Plus, Calendar, DollarSign, Camera, Video, MessageSquare, FileText, Download, FolderOpen, ArrowLeft, Heart, MessageCircle } from "lucide-react";
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          {project.team.map(m => (
            <div key={m.name} className="w-8 h-8 rounded-lg bg-secondary flex items-center justify-center text-xs font-bold text-muted-foreground border border-border -ml-1 first:ml-0" title={m.name}>{m.avatar}</div>
          ))}
        </div>
      </div>
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList className="bg-secondary border border-border">
          ...
          <TabsTrigger value="updates" className="data-[state=active]:bg-card data-[state=active]:shadow-sm">Updates</TabsTrigger>
          <TabsTrigger value="documents" className="data-[state=active]:bg-card data-[state=active]:shadow-sm">Files</TabsTrigger>
          <TabsTrigger value="budget" className="data-[state=active]:bg-card data-[state=active]:shadow-sm">Budget</TabsTrigger>
          <TabsTrigger value="activity" className="data-[state=active]:bg-card data-[state=active]:shadow-sm">Activity</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              <h3 className="font-semibold text-foreground text-sm flex items-center gap-2"><Calendar className="h-4 w-4 text-muted-foreground" /> Deadline</h3>
              <p className="text-3xl font-bold font-display text-foreground">{project.deadline}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              <h3 className="font-semibold text-foreground text-sm flex items-center gap-2"><DollarSign className="h-4 w-4 text-muted-foreground" /> Budget</h3>
              <p className="text-3xl font-bold font-display text-foreground">${(project.spent / 1e6).toFixed(1)}M <span className="text-base font-normal text-muted-foreground">/ ${(project.budget / 1e6).toFixed(1)}M</span></p>
          ...
          <div className="mt-6 rounded-xl border border-border bg-card p-6">
            ...
            <div className="flex flex-wrap gap-3">
              {project.team.map((m) => (
                <div key={m.name} className="flex items-center gap-3 rounded-xl bg-secondary/50 border border-border px-4 py-3">
              ...
              {["Foundation Complete", "Structural Framework", "Exterior Finishing", "Interior Fit-out", "Final Inspection"].map((m, i) => {
                ...
                return (
                  <div key={m} className="flex items-center gap-3">
                    <div className={`w-3 h-3 rounded-full ${done ? "byld-gradient" : "bg-secondary border border-border"}`} />
        ...
        <TabsContent value="tasks">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button size="sm" className="byld-gradient text-primary-foreground border-0 shadow-md"><Plus className="mr-1 h-3 w-3" /> Add Task</Button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {statusColumns.map((col) => {
              const colTasks = projectTasks.filter(t => t.status === col.key);
              return (
                <div key={col.key} className={`rounded-xl border border-border bg-secondary/20 p-3 border-t-2 ${col.color}`}>
                  <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center justify-between">{col.label} <span className="text-xs text-muted-foreground font-normal bg-secondary px-2 py-0.5 rounded-full">{colTasks.length}</span></h4>
                  <div className="space-y-2">
                    {colTasks.map((task) => (
                      <div key={task.id} className="rounded-lg border border-border bg-card p-3 byld-card-hover cursor-pointer group">
        ...
        <TabsContent value="updates">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button size="sm" className="byld-gradient text-primary-foreground border-0 shadow-md"><Plus className="mr-1 h-3 w-3" /> Add Update</Button>
          </div>
              ...
              return (
                <div key={update.id} className="rounded-xl border border-border bg-card p-5 byld-card-hover">
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-lg bg-secondary flex items-center justify-center text-xs font-bold text-muted-foreground shrink-0">{update.uploadedBy.split(" ").map(n => n[0]).join("")}</div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-sm font-semibold text-foreground">{update.uploadedBy}</span>
                        <span className="text-xs text-muted-foreground">{new Date(update.timestamp).toLocaleDateString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
                      </div>
                      <p className="text-sm text-foreground/90 mb-3">{update.description}</p>
                      {(update.type === "photo" || update.type === "video") && (
                        <div className="rounded-lg bg-secondary/50 border border-border h-40 flex items-center justify-center mb-3"><Icon className="h-8 w-8 text-muted-foreground/30" /></div>
                      )}
                      <div className="flex items-center gap-4 pt-2 border-t border-border">
                        <button className="flex items-center gap-1 text-muted-foreground hover:text-primary text-xs transition-colors"><Heart className="h-3.5 w-3.5" /> {update.likes || 0}</button>
                        <button className="flex items-center gap-1 text-muted-foreground hover:text-primary text-xs transition-colors"><MessageCircle className="h-3.5 w-3.5" /> {update.comments?.length || 0}</button>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <TabsContent value="documents">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button size="sm" className="byld-gradient text-primary-foreground border-0 shadow-md"><Plus className="mr-1 h-3 w-3" /> Upload</Button>
          </div>
          <div className="flex gap-2 mb-4 flex-wrap">
            {folders.map((f) => (
              <span key={f} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-secondary/50 border border-border text-muted-foreground cursor-pointer hover:border-primary/30 hover:text-foreground transition-all"><FolderOpen className="h-3 w-3" /> {f}</span>
            ))}
          </div>
          <div className="rounded-xl border border-border bg-card divide-y divide-border">
            {documents.map((doc) => (
              <div key={doc.id} className="flex items-center gap-4 p-4 hover:bg-secondary/20 transition-colors group">
                ...
                <Button variant="ghost" size="icon" className="shrink-0 h-8 w-8 text-muted-foreground hover:text-foreground"><Download className="h-3.5 w-3.5" /></Button>
              </div>
            ))}
          </div>
        </TabsContent>
        ...
        <TabsContent value="budget">
          ...
          <div className="rounded-xl border border-border bg-card p-5 mb-6">
            <h3 className="font-display font-semibold text-foreground text-sm mb-4">Budget Breakdown ($K)</h3>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <Button size="sm" className="byld-gradient text-primary-foreground border-0 shadow-md"><Plus className="mr-1 h-3 w-3" /> Add Expense</Button>
          </div>
          <div className="rounded-xl border border-border bg-card divide-y divide-border">
            {expenses.map((exp) => (
          ...
          <div className="space-y-0">
            {activities.map((a, i) => (
              <div key={a.id} className="flex gap-4">
                ...
                <div className="pb-6">
                  <p className="text-sm text-foreground"><span className="font-medium text-primary">{a.user}</span> <span className="text-muted-foreground">{a.action}</span></p>
                  <p className="text-[10px] text-muted-foreground mt-1">{new Date(a.timestamp).toLocaleDateString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</p>
                </div>
              </div>
            ))}
          </div>
