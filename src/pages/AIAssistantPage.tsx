    ...
    <div className="flex flex-col h-[calc(100vh-3.5rem)]">
      ...
      <div className="h-14 border-b border-border/50 flex items-center px-6 shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg byld-gradient flex items-center justify-center animate-pulse-glow">
        ...
        {messages.map(msg => (
          <div key={msg.id} className={`flex items-start gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
              msg.role === "ai" ? "byld-gradient" : "bg-secondary"
            }`}>
          ...
          <div className="flex flex-wrap gap-2">
            {suggestedPrompts.map(prompt => (
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-secondary/50 border border-border/50 text-xs text-muted-foreground hover:text-foreground hover:border-primary/30 transition-all"
              >
                {prompt}
              </button>
            ))}
      ...
      <div className="p-4 border-t border-border/50">
        <div className="flex items-center gap-2 max-w-3xl mx-auto">
          <Input
            ...
            className="flex-1 h-11 bg-secondary/50 border-border/50 text-sm rounded-xl"
            value={input}
          ...
          <Button
            ...
            className="byld-gradient text-primary-foreground border-0 h-11 w-11 p-0 rounded-xl shadow-lg shadow-primary/20"
          >
