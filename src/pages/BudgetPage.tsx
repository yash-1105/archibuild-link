...
const catColors = ["hsl(217, 91%, 60%)", "hsl(250, 80%, 62%)", "hsl(152, 69%, 40%)", "hsl(38, 92%, 50%)", "hsl(0, 72%, 51%)", "hsl(217, 91%, 60%)"];
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-xl border border-border bg-card p-5">
          <h3 className="font-display font-semibold text-foreground text-sm mb-4">Budget by Project ($K)</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={projectBudgets} barGap={4}>
              <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: 'hsl(220, 10%, 46%)', fontSize: 10 }} />
              <YAxis axisLine={false} tickLine={false} tick={{ fill: 'hsl(220, 10%, 46%)', fontSize: 10 }} />
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie data={categoryData} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={3} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
      ...
      <div>
        ...
        <div className="rounded-xl border border-border bg-card divide-y divide-border">
          {expenses.map(exp => (
            <div key={exp.id} className="flex items-center justify-between p-4 hover:bg-secondary/30 transition-colors">
              <div>
